...
// rugosidade da malha
void pontosAlturaMalha(void)
{
	for (int y = 0; y < LIN; y++)
	{
		for (int x = 0; x < COL; x++)
		{
			terreno[x][y] = rand() % 10 + (-10);
		}
	}
}
		...
		for(int y = 0; y < LIN; y++)
		{
			glBegin(GL_TRIANGLE_STRIP);
				...
				for(int x = 0; x < COL-1; x++)
				{
					...
					glVertex3i(x*scl,y*scl,terreno[x][y]);
					glVertex3i(x*scl,(y+1)*scl,terreno[x][y+1]);
				}
			glEnd();
		}
...
void teclado(unsigned char key, int x, int y)
{
	...
	cout << "valor x: " << obsX << endl; 
	cout << "valor y: " << obsY << endl; 
	cout << "valor z: " << obsZ << endl; 
	glutPostRedisplay();
}
